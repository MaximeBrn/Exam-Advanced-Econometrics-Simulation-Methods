...
source(paste(path,'intcond.R',sep=""))
source(paste(path,'intcondP.R',sep=""))
...
xbin=length(xval) # cardinal
xtran=matrix(0,zbin*xbin,xbin) # (created in xgrid)
xtranc=array(0,c(xbin,xbin,zbin)) #
for(z in 1:zbin){
  temp=xgrid(zval[z],xval)  # call xgrid function
  xtran[(1+(z-1)*xbin):(z*xbin),] = temp$xtran # store transition
...
RX1=cbind(rep(1,zbin*xbin),xvalr,zvalr,xvalr*zvalr,xvalr*xvalr,zvalr*zvalr) # W1t in supplemental
