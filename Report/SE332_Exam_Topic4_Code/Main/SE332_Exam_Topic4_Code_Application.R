...
if(hetero){
  ...
  stargazer(Res,title="Application on Rust's data - engine unobserved",type="latex",out='Output Main/Application_unobserved.tex',flip=TRUE,omit.summary.stat=c("sd","n","min","max"))
} else {
  Res <- as.data.frame(Bccp)
  colnames(Res) <- c("theta_0","theta_1","theta_2","beta")
  stargazer(Res,title="Application on Rust's data - engine observed",type="latex",out='Output Main/Application_observed.tex',flip=TRUE,omit.summary.stat=c("sd","n","min","max"))
}
