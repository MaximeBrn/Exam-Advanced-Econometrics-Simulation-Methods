...
# Objective : estimate the structural parameters but not the beta
...
# Set your working directory in SE332_Exam_Topic4_Code
setwd("~/GitHub/Exam-Advanced-Econometrics-Simulation-Methods/Report/SE332_Exam_Topic4_Code")

hetero = FALSE   # Is heterogeneity unobserved?

source('Functions/xgrid_Rust.R')
source('Functions/wlogitd.R')
source('Functions/wlogit_nobeta.R')
sourceCpp('Functions/fvdataBOTH_Rust.cpp')
source('Functions/intcond.R')
source('Functions/intcondP.R')
...
xccp = cbind(rep(1,N*T),x2,s2) 

beta=alphac[4]

if(!hetero){
  bccp=alphac[1:3]
  bccp = optim(bccp,wlogit_nobeta,Y=y2,X=cbind(xccp,fvt1),Beta=beta,P=rep(1,N*T),method="BFGS")$par
  #bccp = glm(y2~cbind(xccp)-1,family='binomial')$coef #much faster than 'optim'
} else {
  ...
  while(cond==0){
    ...
    U1 = cbind(xccp)%*%bccp+fvt1%*%beta # v2-v1
    ...
    bccp = optim(bccp,wlogit_nobeta,Y=y2,X=cbind(xccp,fvt1),Beta=beta,P=PType,method="BFGS")$par 
...
if(hetero){
  ...
  stargazer(Res,title=paste("Application on Rust's data - beta=",beta," - engine unobserved",sep=""),type="latex",out=paste("Output Main/Application_unobserved_nobeta_",beta*100,".tex",sep=""),flip=TRUE,omit.summary.stat=c("sd","n","min","max"))
} else {
  Res <- as.data.frame(Bccp)
  colnames(Res) <- c("theta_0","theta_1","theta_2")
  stargazer(Res,title=paste("Application on Rust's data - beta=",beta," - engine observed",sep=""),type="latex",out=paste("Output Main/Application_observed_nobeta_",beta*100,".tex",sep=""),flip=TRUE,omit.summary.stat=c("sd","n","min","max"))
}
