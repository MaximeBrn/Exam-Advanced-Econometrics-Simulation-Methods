...
# Set environment
source('Functions/xgrid.R') # Function used to compute the transition matrix
source('Functions/xgrid.R') # Function used to compute the transition matrix
source('Functions/wlogitd.R') # Function used for reduced form logit
source('Functions/wlogit.R') # Function maximized by the structural parameters
sourceCpp('Functions/genbus4.cpp') # Function used to generate the data
sourceCpp('Functions/fvdataBOTH.cpp') # Function used to compute value function part of the likelihood
source('Functions/intcond.R') # Used to compute initial conditions
source('Functions/intcondP.R') # Used to compute q_ns
...
if(hetero){
  ...
  stargazer(Res,title="Simulation - s unobserved",type="latex",out='Output Main/Simulation_unobserved.tex',flip=TRUE,omit.summary.stat=c("min","max"))
  }else{
  Res <- as.data.frame(Bccp)
  colnames(Res) <- c("theta_0","theta_1","theta_2","beta")
  stargazer(Res,title="Simulation - s observed",type="latex",out='Output Main/Simulation_observed.tex',flip=TRUE,omit.summary.stat=c("min","max"))
  }
